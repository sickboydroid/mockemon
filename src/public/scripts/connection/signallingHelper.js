...
// todo modify
export async function onLocalIceCandidateUpdate(event, socket) {
  if (event.candidate) {
...
// todo modify
export function onRemoteIceCandidateUpdate(peerConnection, iceCandidates) {
  for (const iceCandidate of iceCandidates) {
    if (remoteIceCandidates.has(iceCandidate)) continue;
    peerConnection.addIceCandidate(new RTCIceCandidate(iceCandidate));
    remoteIceCandidates.add(iceCandidate);
    console.debug("New ice candidates added from remote");
  }
}
...
export async function setup(socket) {
  const role = sessionStorage.getItem("role");
  if (role === "caller") {
    await setupConnectionAsCaller(socket);
  } else if (role === "callee") {
    await setupConnectionAsCallee(socket);
  } else {
